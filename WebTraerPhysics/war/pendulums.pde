...
{ 
  ...
  for (int i=0; i<n; i++) 
    for (int j=i+1; j<n; j++) 
        physics.replaceAttraction( f[i][j], new Attraction(p[i], p[j], kVal, 1) ); // replace that Attraction
}
      ...
      if (i0 < 50)
        {
          ...
          for (int i=0; i<n; i++)
              physics.replaceAttraction( pf[i], new Attraction(perturbation, p[i], kVal0, 1) ); 
        }  
      else
        {
...
{ 
  ...
  if (mouseY < height/2) 
    ...
    { // Otherwise, the mouse is in the lower half - create a new perturbation
      ...
      for (int i=0; i<n; i++)
          if (replace)
             physics.replaceAttraction( pf[i], new Attraction(perturbation, p[i], kVal, 1) );
          else   
